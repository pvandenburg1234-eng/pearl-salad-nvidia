...
| `POWER_TEMP_MAX` | `88` °C. Temperature ceiling for the periodic check, a fallback for hosts whose driver reports the slowdown flags as `[N/A]`. Laptop GPUs report no power limit at all (`[N/A]`), so on them this ceiling and the **hardware** slowdown flag are the whole protection. The **software** slowdown flag is not counted on laptops (since v1.5.1): laptop firmware sets it while holding its temperature target, and a 5080 Laptop reporting it sat at 123 to 127 W, 84 °C and 105 TH/s. It is logged once as `software thermal slowdown reported`. On desktop cards (limit reported) either flag still counts. `0` disables the ceiling. |
...
| `host check: software thermal slowdown reported ... not counted` (laptop) | Informational. The laptop's firmware is holding its temperature target, which is how every 5080 Laptop host runs at 84 to 86 °C. Only the hardware flag or `POWER_TEMP_MAX` will reject a laptop. If a laptop host settles at clearly bad watts without tripping either (a 5070 Ti Laptop fell 95 → 72 W at 86 °C), it stays; judge it by the pool hashrate and reallocate it by hand. |
...
- **Hashrate floor (`HASHRATE_MIN`).** A per-group minimum in TH/s: a worker
  ...
  today: an RTX 5080 Laptop pinned by its owner's power profile at 95 W and
  96 TH/s, cool at 66 °C, next to another at 175 W and 130 TH/s (laptops
  report no power limit, so the cap is invisible to the power checks and
  nothing is throttling), and an RX 9070 XT at 105 TH/s in a group doing
  119 to 135 (AMD exposes no watts or temperature under Salad). Both were
  still profitable, and the classes had no spare machines to swap to, so
  this stays unbuilt for now. When built it would sit in `host_check_tick`
  next to the power checks and use `parse_hashrate` on the miner log; the
  floor is per class, so it is a group variable, not a default.
...
| Version | Date | Notes |
|---|---|---|
| v1.6.1 | 2026-09-26 | Log shipping posts at once (and waits up to 5 s for the post) before asking Salad to reallocate, and first thing on SIGTERM. v1.6.0 lost the last seconds of a rejected node's log - the lines that say why it was rejected - because Salad stops the container before the next 10 s post (seen on 5070 Ti Laptop machine 323f6072, whose network cannot complete TLS to the pool). Same scripts as pearl-salad v1.4.1. |
| v1.6.0 | 2026-09-26 | In-container log shipping to Axiom (`AXIOM_TOKEN`, off with `LOG_SHIP=0`), because Salad's external-logging forwarder has delivered nothing since 12:32 UTC 25 Sep while the portal and the containers' own network work. Same `common.sh`/`entrypoint.sh`/`bench.sh` as pearl-salad v1.4.0. |
| v1.5.1 | 2026-09-25 | Laptop rule for the periodic host check: on hosts that report no power limit, the software thermal slowdown flag no longer counts as a bad reading (logged once instead); the hardware flag and `POWER_TEMP_MAX` still do. v1.5.0 rejected a healthy 5080 Laptop host (123 to 127 W, 84 °C, 105 TH/s, shares accepted) after three minutes purely on that flag, which laptop firmware sets while holding its temperature target. Desktop behaviour unchanged. |
